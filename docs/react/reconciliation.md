...
## Diffing 알고리즘 (Heuristics)

- React가 비교를 O(n)으로 처리할 수 있는 이유는, 두 가지 가정을 따르기 때문

### 1. 엘리먼트 타입이 다르면, 하위 트리도 전부 파괴한다.

- 해당 엘리먼트부터 파괴하고 새로 트리를 생성(아래로 전부 unmount)
- 같을 경우에는 변경된 내용만 업데이트(rerendering)

### 2. 'key' prop을 통해 리스트의 변경 사항을 파악한다.

- 배열로 컴포넌트를 렌더링 하는 경우는 매우 흔한데, key를 통해 기존 트리와 이후 트리의 자식들이 일치하는지 확인
- **key가 변경 되지 않으면 그 컴포넌트는 다시 실행하지 않음**
- map()을 돌릴 때 key를 쓰라고 경고가 뜨는 이유
