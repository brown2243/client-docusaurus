...
바닐라 자바스크립트의 중요성을 강조하게 되는 주범이기도 하다.
...
기존에는 `form`, `submit`으로 데이터를 보내면, 결과가 포함된 완성된 HTML 문서 전체를 받아야 했다(동기적으로 동작).

반면에 `AJAX`를 사용하면 응답(주로 `JSON`)만 받아서 ui를 수정하면 끝인 것이다. `SPA` 의 기반 기술이기도 하다.

web 환경에서는 `XMLHttpRequest` 객체(이벤트 방식)를 기반으로, node에서는 `http` 모듈을 기반으로 구현된다.
...
인터프리터 언어는 일반적으로 컴파일 언어에 비해 속도가 느린데, 실행 속도를 향상 시키기 위해 최적화 기능이 있는 JIT 컴파일러를 도입한 것이다.
...
이렇게 본인이 생각하는 웹 개발의 발전 과정을 정리해 보았다. 내용이 방대하다 보니 생각보다 작성하는데 오래 걸렸다.
